...
func (p Profile) EnumerateDevices() (resourceslice.DriverResources, error) {
	...
	bandwidthCapacity := resourceapi.DeviceCapacity{
		Value: resource.MustParse("100G"),
		RequestPolicy: &resourceapi.CapacityRequestPolicy{
			Default: ptr.To(resource.MustParse("1G")), // equal division 100G / 100
			ValidRange: &resourceapi.CapacityRequestPolicyRange{
				Min:  ptr.To(resource.MustParse("100M")), // prevent zero-consuming
				Max:  ptr.To(resource.MustParse("100G")), // optional
				Step: ptr.To(resource.MustParse("1M")),
			},
		},
	}
